...
Drivetrain::Drivetrain() : frc::Subsystem("Drivetrain") {
	...
	m_right_1.SetSensorPhase(false);
	m_left_2.SetSensorPhase(false);
	m_right_2.SetSensorPhase(false);
...
void Drivetrain::ResetEncoders() {
  ...
  m_left_2.GetSensorCollection().SetIntegratedSensorPosition(0);
  m_right_2.GetSensorCollection().SetIntegratedSensorPosition(0);
  previous_distance = 0;
}
...
double Drivetrain::GetDistance() {
  ...
  right_distance = (m_right_1.GetSensorCollection().GetIntegratedSensorPosition() + m_right_2.GetSensorCollection().GetIntegratedSensorPosition()) / 2;
	frc::SmartDashboard::PutNumber("Left drive encoders", left_distance);
	frc::SmartDashboard::PutNumber("Right drive encoders", right_distance);
  return left_distance;//(((pi * kDrivetrainWheelDiameter * kDrivetrainGearRatio) / kFalconTicksPerRevolution) * ((left_distance + right_distance) / 2));
}
...
void Drivetrain::IncrementXZ() {
	frc::SmartDashboard::PutNumber("Delta distance", GetDistance() - previous_distance);
  linear_displacement = 2 * (GetDistance() - previous_distance) * abs(sin((GetAngle() - starting_angle) * pi/180.0) / ((GetAngle() - starting_angle) * pi/180.0));
  x_position += sin(GetAngle() * pi/180.0) * linear_displacement;
  z_position += cos(GetAngle() * pi/180.0) * linear_displacement;
