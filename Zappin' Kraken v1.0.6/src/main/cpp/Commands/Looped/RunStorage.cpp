...
void RunStorage::Initialize() {
  ...
  manual_vertical_was_pressed = Robot::m_oi.GetOperatorButton(3);
}

void RunStorage::Execute() {
  if (Robot::m_storage.GetShooting() == false) {
    if (Robot::m_oi.GetOperatorButton(3) == true) {
      Robot::m_storage.SetVerticalSpeed(kVerticalStorageSpeed);
      Robot::m_storage.SetBallCount(0);
      m_timer.Reset();
      m_timer.Stop();
      manual_vertical_was_pressed = true;
    } else if (Robot::m_storage.GetManualControl() == true) {
      Robot::m_storage.SetVerticalSpeed(0);
      Robot::m_storage.SetBallCount(0);
      m_timer.Reset();
      m_timer.Stop();
    } else {
      if (manual_vertical_was_pressed == true) {
        Robot::m_storage.SetVerticalSpeed(0);
      }
      manual_vertical_was_pressed = false;
