...
void DriveToPoint::Execute() {
  ...
  if (ending_speed == 0) {
    ...
    if (abs(linear_speed) > abs(starting_speed)) {
      linear_speed = abs(starting_speed) * (abs(linear_speed) / linear_speed);
    }
  ...
  frc::SmartDashboard::PutNumber("Kahuna Angle Target", angle_target);
  frc::SmartDashboard::PutNumber("Kahuna Angle + Angle PID Target", angle_target + PID_angle_target);
  frc::SmartDashboard::PutNumber("Translated X", translated_x);
  frc::SmartDashboard::PutNumber("Translated Z", translated_z);
  frc::SmartDashboard::PutNumber("X Difference", x_difference);
  frc::SmartDashboard::PutNumber("Z Difference", z_difference);
}
